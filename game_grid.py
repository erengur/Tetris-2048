...
def checkRows(self):
   ...
   # Method used for implementation the merge and control the grid with indexes
   def merge(self):
      row_ = len(self.tile_matrix) - 1
      col_ = len(self.tile_matrix[0])
      for row in range(row_):
         for col in range(col_):
            # the grid is merge by a tile_matrix if it is not None
            if self.tile_matrix[row][col] is not None and self.tile_matrix[row-1][col]is not None:
               if self.tile_matrix[row][col].number == self.tile_matrix[row - 1][col].number:
                  self.tile_matrix[row - 1][col].number = 2 * self.tile_matrix[row][col].number # doubled matrix
                  self.score = self.score + self.tile_matrix[row - 1][col].number # updating score
                  self.color(row - 1, col) # updating color
                  self.tile_matrix[row][col] = None 
   ...
   # checking if the game is over due to having tiles above the top-most grid row.
